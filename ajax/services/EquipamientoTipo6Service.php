    ...
    public function calcular(int $proyectoId): array
    {
        ...
        $aplicar = function (string $etiqueta, array $tabla, int $dotaciones) use (&$acum) {
            ...
            foreach ($tabla as $eid => $cantBase) {
                ...
                if ($sub <= 0) continue;
                if (!isset($acum[$eid])) $acum[$eid] = ['cantidad' => 0, 'por_regla' => []];
                $acum[$eid]['cantidad'] += $sub;
                $acum[$eid]['por_regla'][] = ['regla' => $etiqueta, 'cantidad_base' => (int)$cantBase, 'dotaciones' => $dotaciones, 'subtotal' => $sub];
            }
        };
        ...
        foreach ($acum as $eid => $info) {
            $equipos[] = ['equipo_id' => $eid, 'nombre_equipo' => $nombres[$eid] ?? null, 'origen' => 'norma_upc', 'cantidad' => $info['cantidad'], 'por_regla' => $info['por_regla']];
        }

        return [
            'camas'      => ['uci' => $camasUCI, 'uti' => $camasUTI],
            'dotaciones' => ['uci' => $dotacionesUCI, 'uti' => $dotacionesUTI, 'enfermeria_uci' => $dotacionesEnfUCI, 'enfermeria_uti' => $dotacionesEnfUTI],
            'equipos'    => $equipos,
        ];
    }
}